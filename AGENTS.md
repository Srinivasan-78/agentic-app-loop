<!--
  ...
  Fingerprint: AMK1.1Qo1kgnHNSbKTQKj1XmWTw
-->
# Repo rules
...
- Editing a file makes its fingerprint stale. That is expected. Refresh it with
  `node authormark.mjs stamp <file>` (using the AuthorMark engine from
  `Srinivasan-78/authormark-watch`) — never resolve staleness by deleting the header.
